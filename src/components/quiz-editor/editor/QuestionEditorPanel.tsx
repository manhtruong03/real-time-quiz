...
import SortableAnswerList from './answer-editors/SortableAnswerList'; // <<< Import Jumble editor
...
const QuestionEditorPanel: React.FC<QuestionEditorPanelProps> = ({
    ...
    // Helper checks (keep or refine as needed)
    ...
    return (
        <div className={cn("flex-grow flex flex-col gap-4", className)}>
            ...
            <div className="flex-grow mt-2 p-4 border rounded-lg bg-background/50 flex flex-col justify-start min-h-[250px]"> {/* Increased min-height slightly */}
                {/* Conditional Rendering for Answer Editors */}
                {(isQuiz || isSurvey) ? (
                    <QuizSurveyAnswerEditor />
                ) : isJumble ? (
                    // <<< Render Jumble editor >>>
